...
{
    if (windowWidth <= 0.0f || scrollAmount == 0.0f) return;
    ...
    float visibleWidth = 1.0f / oldZoom;
    float cursorPos = offset + cursorNorm * visibleWidth;
    offset = cursorPos - cursorNorm / zoomFactor;
...
void Scrubber::draw()
{
    ...
    glVertex2f((float)vp->width(), scrubberY1);
    ...
    if (vp->durationSeconds() > 0.0) {
        ...
        if (x + playheadWidth / 2 >= 0.0f && x - playheadWidth / 2 <= vp->width()) {
            ...
            glVertex2f(x + playheadWidth / 2, scrubberY0);
            glVertex2f(x + playheadWidth / 2, scrubberY1);
            glVertex2f(x - playheadWidth / 2, scrubberY1);
            glEnd();
        }
    }
